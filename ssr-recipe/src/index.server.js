...
// asset-manifest.json에서 파일 경로를 조회한다.
const manifest = JSON.parse(
    fs.readFileSync(path.resolve('./build/asset-manifest.json'), 'utf8')
);

const chunks = Object.keys(manifest.files)
    .filter(key => /chuck\.js$/.exec(key)) // chuck.js로 끝나는 키를 찾아서
...
const serverRender = async (req, res, next) => {
    ...
    ReactDOMServer.renderToStaticMarkup(jsx); // 한번 렌더링하고,
...
app.use(serve); // 순서 중요하다 serverRender 전에 위치해야 한다.
